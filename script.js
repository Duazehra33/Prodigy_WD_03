...
Game grid is represented in the array Grid.cells as follows:

[0] [1] [2]
[3] [4] [5]
[6] [7] [8]

Cells (array elements) hold following numeric values:
0 = not occupied, 1 = player, 3 = computer.
Allows us to quickly get overview of game state:
ie.
if sum of all cells in row = 9, COMPUTER WINS
if sum of all cells in row = 3 & all cells occupied, HUMAN PLAYER WINS
etc.

(2) Strategy of makeComputerMove()

Computer first looks for almost completed rows, columns,
diagonals, where 2 fields occupied either by the HUMAN PLAYER
or by the COMPUTER. 
Computer can win by completing sequence, it continues to do so; 
if it can block the player from winning with the next move, it does so. 
If none of above applies, compuater plays center field (if free), 
otherwise selects random free field. 
NOTE: Not 100% winnable strategy, but gameplay experience is enjoyable.
...
function handleExitClick() {
    if (confirm("Are you sure you want to exit?")) {
      window.location.href = "index.html"; // Replace with your desired exit route
    }
  }
  
  function handlePlayAgainClick() {
    restartGame(false); // Don't ask for role assignment
  }
...
function announceWinner(text) {
    ...
    setTimeout(closeModal, 3500, "winAnnounce");
}
...
function endGame(who) {
    ...
    if (who == player) {
        ...
        document.getElementById("winAnnounce").style.background = "orange", document.getElementById("tieAnimation").style.display = "block", document.getElementById("winAnimation").style.display = "none", document.getElementById("lossAnimation").style.display = "none";
    }
